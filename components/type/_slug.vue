...
export default {
  ...
  async asyncData ({ route, $appsyncClient, error }) {
    try {
      const { zipcode } = route.query
      let dealers = []
      if (zipcode && !Number.isNaN(+zipcode)) {
        const dealersResponse = await $appsyncClient
          .query({
            query: gql(queries.searchDealers),
            variables: {
              filter: {
                zip: {
                  eq: +zipcode
        ...
        dealers = dealersResponse.data.searchDealers.items.map((item) => {
  ...
  methods: {
    fetchData () {
      ...
      this.$appsyncClient
        .query({
          query: gql(queries.listV2FlatRecallsByType),
          variables: {
            type: slug,
        ...
        .then(({ data }) => {
          const recalls = data.listV2FlatRecallsByType.items
            .map((item) => {
              item.description = item.summary
              item.remedy = item.Remedy
              item.risk = item.Conequence
              item.id = item.recall_id
              if (item.recallDate && item.recallDate.length === 8) {
                // YYYYMMDD ==> MM/DD/YYYY
                const day = item.recallDate.substr(6, 2)
                const month = item.recallDate.substr(4, 2)
                const year = item.recallDate.substr(0, 4)
          ...
          this.nextToken = data.listV2FlatRecallsByType.nextToken
        })
        .catch(console.error)
        .finally(() => {
          this.showLoader = false
